...
The following Mermaid diagram represents the module-level class structure of the project. In C, "classes" are represented as structs + function groups per module.
...
classDiagram
    ...
    class AppState {
        +GtkApplication* app
        +GtkWidget* window
        +GtkTreeStore* key_store
        +GtkListStore* value_store
        +HKEY current_hkey
        +gchar* current_path
    }
    ...
    class Main {
        +main(argc, argv) int
        +on_app_activate(app, state) void
    }

    class RegistryView {
        +view_build(state) void
        +view_set_status(state, msg) void
        +view_show_safety_notice(state) void
    }

    class RegistryModel {
        +model_populate_root_keys(state) void
        +model_populate_subkeys(state, iter, hkey) void
        +model_populate_values(state, hkey) void
    }

    class RegistryController {
        +controller_connect_signals(state) void
        +controller_on_selection_changed(sel, state) void
        +controller_on_row_expanded(view, iter, path, state) void
        +controller_start_auto_refresh(state, interval) void
    }

    class RegistryFacade {
        +registry_open_key(parent, subkey, out_key) BOOL
        +registry_enum_subkeys(hkey, callback, userdata) void
        +registry_enum_values(hkey, callback, userdata) void
        +registry_close_key(hkey) void
        -wchar_to_utf8(wstr) gchar*
        -format_value(type, data, size) gchar*
    }
    ...
    Main --> AppState : creates & owns
    Main --> RegistryView : initializes
    Main --> RegistryModel : initializes
    Main --> RegistryController : initializes

    RegistryView --> AppState : reads widget refs
    RegistryView ..> RegistryTypes : uses types

    RegistryModel --> AppState : populates stores
    RegistryModel --> RegistryFacade : calls facade APIs
    RegistryModel ..> RegistryTypes : uses types

    RegistryController --> AppState : manages state
    RegistryController --> RegistryModel : triggers model updates
    RegistryController --> RegistryView : updates status bar

    RegistryFacade ..> RegistryTypes : uses types
    RegistryFacade : Win32 Registry API boundary
```

### 12) Safety Model (Read-Only Guarantee)
...
The diagram below illustrates the complete module structure and inter-module dependencies of the Registry Viewer. Each box represents a C module (source/header pair), with its key public functions listed.
...
classDiagram
    ...
    class main_c {
        <<Entry Point>>
        +main(argc, argv) int
        +on_activate(app, state) void
        -init_app_state() AppState
        -cleanup_app_state(state) void
    }

    class registry_view_c {
        <<View Layer - GTK Only>>
        +view_build(state) void
        +view_set_status(state, message) void
        +view_show_safety_notice(state) void
        -build_key_tree_pane(state) GtkWidget
        -build_value_list_pane(state) GtkWidget
        -build_toolbar(state) GtkWidget
    }

    class registry_model_c {
        <<Model Layer - Store Population>>
        +model_populate_root_keys(state) void
        +model_populate_subkeys(state, parent_iter, hkey) void
        +model_populate_values(state, hkey) void
        -model_clear_children(store, iter) void
        -model_append_value_row(store, name, type, data) void
    }

    class registry_controller_c {
        <<Controller Layer - Signals and Timers>>
        +controller_connect_signals(state) void
        +controller_start_auto_refresh(state, interval_sec) void
        +controller_stop_auto_refresh(state) void
        -on_selection_changed(selection, state) void
        -on_row_expanded(view, iter, path, state) void
        -on_refresh_tick(state) gboolean
    }

    class registry_facade_c {
        <<Facade - Win32 Boundary>>
        +registry_open_key(parent, subkey, out_hkey) BOOL
        +registry_enum_subkeys(hkey, cb, userdata) void
        +registry_enum_values(hkey, cb, userdata) void
        +registry_close_key(hkey) void
        -wchar_to_utf8(wstr) gchar
        -format_dword(data, size) gchar
        -format_qword(data, size) gchar
        -format_binary(data, size) gchar
        -format_multi_sz(data, size) gchar
    }

    class registry_types_h {
        <<Shared Header>>
        +struct AppState
        +struct RegistryValue
        +enum RegValueType
        +KEY_COLUMN_NAME int
        +VALUE_COLUMN_NAME int
        +VALUE_COLUMN_TYPE int
        +VALUE_COLUMN_DATA int
    }

    main_c --> AppState : creates and manages lifetime
    main_c --> registry_view_c : calls view_build()
    main_c --> registry_model_c : calls model_populate_root_keys()
    main_c --> registry_controller_c : calls controller_connect_signals()

    registry_view_c --> AppState : reads and writes widget refs
    registry_view_c ..> registry_types_h : uses column constants

    registry_model_c --> AppState : populates key_store and value_store
    registry_model_c --> registry_facade_c : enumerates subkeys and values
    registry_model_c ..> registry_types_h : uses RegistryValue and column IDs

    registry_controller_c --> AppState : updates current_hkey and path
    registry_controller_c --> registry_model_c : triggers model refresh
    registry_controller_c --> registry_view_c : calls view_set_status()
    registry_controller_c ..> registry_types_h : uses AppState fields

    registry_facade_c ..> registry_types_h : uses RegistryValue struct
```

> **Read-Only Guarantee:** `registry_facade_c` is the **sole** module that touches Win32 Registry APIs. It deliberately omits `RegSetValueEx`, `RegDeleteKey`, `RegCreateKey`, and all other write/delete calls. All other modules are Registry-write-safe by construction.
